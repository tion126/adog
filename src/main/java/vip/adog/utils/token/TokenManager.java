...
package vip.adog.utils.token;

import vip.adog.common.BaseController;
import vip.adog.common.Constant;
import vip.adog.utils.StrKit;
...
public class TokenManager {
	...
	public static void init(ITokenCache tokenCache) {
		...
		long halfTimeOut = Constant.MIN_SECONDS_OF_TOKEN_TIME_OUT * 1000 / 2;	// Token最小过期时间的一半时间作为任务运行的间隔时间
	 ...
	 */
	private static void createTokenHiddenField(BaseController controller, String tokenName, String tokenId) {
		StringBuilder sb = new StringBuilder();
		sb.append("<input type='hidden' name='").append(tokenName).append("' value='" + tokenId).append("' />");
		controller.setAttr("token", sb.toString());
	}

	private static void createTokenUseTokenIdGenerator(BaseController controller, String tokenName, int secondsOfTimeOut) {
		if (secondsOfTimeOut < Constant.MIN_SECONDS_OF_TOKEN_TIME_OUT)
			secondsOfTimeOut = Constant.MIN_SECONDS_OF_TOKEN_TIME_OUT;
	 ...
	 */
	public static synchronized boolean validateToken(BaseController controller, String tokenName) {
